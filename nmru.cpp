//
// Created by Shreyas Maitreya on 10/8/23.
//
#include <algorithm>
#include <cassert>
#include <map>
...
uint32_t CACHE::find_victim(uint32_t triggering_cpu, uint64_t instr_id, uint32_t set, const BLOCK* current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    ...
    // Find the way whose last use cycle is not the most recent
    auto most_recent = std::max_element(begin, end);
    assert(begin <= most_recent);
    assert(most_recent < end);

    // Find the way whose last use cycle is not the most recent
    auto victim = std::min_element(begin, most_recent);
    assert(begin <= victim);
    assert(victim < most_recent);
